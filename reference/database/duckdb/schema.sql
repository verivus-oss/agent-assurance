...
CREATE TABLE dagtoml.reference_contract (
    singleton_id INTEGER PRIMARY KEY CHECK (singleton_id = 1),
    contract_bundle_sha256 TEXT NOT NULL UNIQUE CHECK (length(contract_bundle_sha256) = 64 AND regexp_full_match(contract_bundle_sha256, '[0-9a-f]{64}')),
    projection_version INTEGER NOT NULL CHECK (projection_version = 1)
);

CREATE TABLE dagtoml.runtime_document (
    instance_file_id UUID PRIMARY KEY NOT NULL,
    template_kind TEXT NOT NULL,
    source_toml BLOB NOT NULL,
    projection_version INTEGER NOT NULL CHECK (projection_version = 1),
    contract_bundle_sha256 TEXT NOT NULL REFERENCES dagtoml.reference_contract(contract_bundle_sha256),
    ...
    CONSTRAINT runtime_document_parent_kind FOREIGN KEY (instance_file_id, template_kind)
        REFERENCES dagtoml.instance_file(id, template_kind),
...
CREATE VIEW dagtoml.adapter_contract_document AS
SELECT d.*, f.source_path, f.content_sha256, f.framework_profile, f.title
FROM dagtoml.runtime_document AS d
JOIN dagtoml.instance_file AS f ON f.id = d.instance_file_id
WHERE d.template_kind = 'adapter-contract';

CREATE VIEW dagtoml.adapter_registry_binding_document AS
SELECT d.*, f.source_path, f.content_sha256, f.framework_profile, f.title
FROM dagtoml.runtime_document AS d
JOIN dagtoml.instance_file AS f ON f.id = d.instance_file_id
WHERE d.template_kind = 'adapter-registry-binding';

CREATE VIEW dagtoml.gate_decision_document AS
SELECT d.*, f.source_path, f.content_sha256, f.framework_profile, f.title
FROM dagtoml.runtime_document AS d
JOIN dagtoml.instance_file AS f ON f.id = d.instance_file_id
