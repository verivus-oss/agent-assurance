...
# Round-2 review found that divergence.
...
def canonical_bound_tuple(doc: dict) -> str:
    ...
    this function and was the subject of a round-1 review blocker. Inlining
    ...
    one `binds_sha256` could bind two distinct mutations. Codex demonstrated
    exactly that against the first implementation, with a newline-bearing
...
def validate_one(path: pathlib.Path, repo_root: pathlib.Path) -> list[str]:
    ...
    # Round-4 review found it. The primaries already report these cleanly
    # through the round-2 three-state accessor.
